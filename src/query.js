...
export async function parquetQuery(options) {
  ...
  if (typeof orderBy === 'string') {
    ...
    const readOrderTime = performance.now() - startTime
    ...
    const sortOrderTime = performance.now() - startTime
    startTime = performance.now()
    ...
    const totalTime = performance.now() - startStartTime
    console.log(`parquetQuery: ${readOrderTime.toFixed(0)}ms + ${sortOrderTime.toFixed(0)}ms + ${fetchTime.toFixed(0)}ms = ${totalTime.toFixed(0)}ms`)
    return data
  } else {
...
async function parquetReadRows(options) {
  ...
  const includedRowGroups = groupIncluded.filter(Boolean).length
  const rowGroupPercent = 100 * includedRowGroups / groupIncluded.length
  ...
  const readRowsTime = performance.now() - startTime
  console.log(`parquetReadRows: groups ${includedRowGroups}/${groupIncluded.length} (${rowGroupPercent.toFixed(1)}%) ${readRowsTime.toFixed(0)}ms`)
  return sparseData
}
